...
export default function Infos({ product, setActiveImg }) {
  ...
  const addToCartHandler = async () => {
    ...
    if (qty > data.quantity) {
      setError(
        "The Quantity you have choose is more than in stock. Try and lower Qty"
      );
    } else if (data.quantity < 1) {
      setError("Sorry. This product is out of stock ");
      return;
    } else {
      let _uid = `${data._id}_${product._id}_${product.style}_${router.query.size}`;
    ...
    <div className={styles.infos}>
      <div className={styles.infos__container}>
        ...
        <div className={styles.infos__rating}>
          ...
          />
          {product.numReviews}
          {product.numReviews === 1 ? " review" : " rewiews"}
        </div>
